...
async fn inspect(hash: &str, eth_rpc_url: &str) -> Result<(), Box<dyn std::error::Error>> {
	...
	if logs.is_empty() {
		...
		for (i, log) in logs.iter().enumerate() {
			...
			let raw_topics = log.get("topics").and_then(|v| v.as_array()).unwrap_or(&no_topics);
			let data_str = log.get("data").and_then(|v| v.as_str()).unwrap_or("0x");
			...
			if let Some(decoded) = decode_log(&topics, &data) {
				...
				let selector_label = raw_topics.first().and_then(|t| t.as_str()).and_then(|t| {
